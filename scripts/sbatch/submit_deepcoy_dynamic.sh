#!/bin/bash
# =============================================================================
# Dynamic DeepCoy Job Submission - Load Balancing Version
#
# Features:
...
while true; do
    ...
    # Use find for faster traversal if possible, but glob is fine for ~800 dirs
    for dir in ${SHARED_AFFINITY_DIR}/uniprot_*; do
        if [ -d "$dir" ]; then
            ...
            if [ -f "${dir}/deepcoy_output/${uid}_generated_decoys.txt" ]; then
                ...
                # Check if input parquet exists (valid target)
    ...
    echo "Status: ${COMPLETED_COUNT} completed, ${TOTAL_PENDING} pending."
